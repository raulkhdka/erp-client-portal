        ...
        <div class="col-md-10">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('employee.documents.update', $document) }}" method="POST" enctype="multipart/form-data" id="editForm">
                        @csrf
                        @method('PUT')

                        <!-- File Upload Area -->
                        <div class="mb-4">
                            <label class="form-label">Document File <span class="text-muted">(Leave blank to keep current file)</span></label>
                            <div class="upload-area" id="uploadArea">
                                <i class="fas fa-cloud-upload-alt fa-3x text-primary mb-3"></i>
                                <h5>Drag and drop your new file here</h5>
                            ...
                            <div id="filePreview" class="file-preview">
                                <div class="d-flex align-items-center">
                                    ...
                                    <div>
                                        <strong id="fileName">{{ $document->file_name }}</strong><br>
                                        <small id="fileSize" class="text-muted">{{ $document->formatted_file_size }}</small>
                                    </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('categories_id') is-invalid @enderror"
                                        ...
                                        @foreach($categories as $id => $name)
                                            <option value="{{ $id }}" {{ old('categories_id', $document->category_id) == $id ? 'selected' : '' }}>{{ $name }}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select client-select @error('client_id') is-invalid @enderror"
                                    ...
                                    @foreach($clients as $id => $name)
                                        <option value="{{ $id }}" {{ old('client_id', $document->client_id) == $id ? 'selected' : '' }}>
                                            {{ $name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="permission-section mb-4">
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-md-6">
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" id="is_confidential" name="is_confidential" value="1" {{ old('is_confidential', $document->is_confidential) ? 'checked' : '' }}>
                                        <label class="form-check-label" for="is_confidential"><strong>Confidential Document</strong></label>
                                    </div>
                                </div>
                            </div>

                            {{-- <div id="specificUsers" style="{{ old('is_public', $document->is_public) ? 'display: none;' : '' }}">
                                        ...
                                        <div class="col-md-4 mb-2">
                                            <div class="form-check">
                                                <input class="form-check-input" type="checkbox" name="access_permissions[]"
                                                       ...
                                                       {{ in_array($id, old('access_permissions', $document->access_permissions ?? [])) ? 'checked' : '' }}>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            removeFile.addEventListener('click', function() {
                ...
                titleInput.value = $document->title; // Revert to original title
            });
