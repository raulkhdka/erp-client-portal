...
@section('title', 'Employee Dynamic Form Details')

@section('breadcrumb')
    <a href="{{ route('employees.dynamic-forms.index') }}">My Forms</a>
    <span class="breadcrumb-item active">{{ $form->name }}</span>
@endsection

@section('actions')
    <div class="btn-group">
        <a href="{{ route('employees.dynamic-forms.edit', $form->id) }}" class="btn btn-warning">
            <i class="fas fa-edit me-2"></i>Edit Form
        </a>
        <a href="{{ route('admin.dynamic-forms.public-show', $form->id) }}" target="_blank" class="btn btn-info">
...
@section('content')
@unless (Auth::id() == $form->employee_id)
...
@else
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        ...
        <div class="btn-toolbar mb-2 mb-md-0">
            {{-- <a href="{{ route('employees.dynamic-forms.index') }}" class="btn btn-outline-secondary me-2">
                <i class="fas fa-arrow-left me-2"></i>Back to My Forms
            </a>
            <a href="{{ route('employees.dynamic-forms.edit', $form->id) }}" class="btn btn-warning text-white me-2">
                <i class="fas fa-edit me-2"></i>Edit Form
            </a>
            <a href="{{ route('admin.dynamic-forms.public-show', $form->id) }}" target="_blank" class="btn btn-info me-2">
                <i class="fas fa-share-alt me-2"></i>View Public Form
            </a> --}}
            {{-- Uncomment if employees are allowed to delete their forms --}}
            <form action="{{ route('employees.dynamic-forms.destroy', $form->id) }}" method="POST" class="d-inline-block" onsubmit="return confirm('Are you sure you want to delete this form and all its associated fields and responses?');">
                @csrf
                @method('DELETE')
        ...
        <div class="col-md-6">
            <div class="card shadow mb-4">
                ...
                <div class="card-body">
                    ...
                    {{-- Uncomment to display settings if needed
                                ...
                                <li class="list-group-item d-flex justify-content-between align-items-center">
                                    <div>
                                        ...
                                        <small class="text-muted">
                                            ...
                                            @if ($field->field_options) | Options: {{ implode(', ', json_decode($field->field_options, true) ?? [$field->field_options]) }} @endif
                                        </small>
                                    </div>
                        ...
                        <tbody>
                            @foreach ($form->responses as $response)
                                <tr>
                                    ...
                                    <td>
                                        @if ($response->client)
                                            <a href="{{ route('employees.clients.show', $response->client->id) }}">{{ $response->client->user->name ?? $response->client->company_name }}</a>
                                        @else
                                            Guest
                                        @endif
                                    </td>
