...
@section('breadcrumb')
    <a href="{{ route('admin.dynamic-forms.index') }}">Dynamic Forms</a>
    <span class="breadcrumb-item active">{{ $form->name }}</span>
@endsection

@section('actions')
    <div class="btn-group">
        <a href="{{ route('admin.dynamic-forms.edit', $form->id) }}" class="btn btn-warning">
...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
    ...
    <div class="btn-toolbar mb-2 mb-md-0">
        <a href="{{ route('admin.dynamic-forms.index') }}" class="btn btn-outline-secondary me-2">
            <i class="fas fa-arrow-left me-2"></i>Back to Forms
        </a>
        <a href="{{ route('admin.dynamic-forms.edit', $form->id) }}" class="btn btn-warning text-white me-2">
            <i class="fas fa-edit me-2"></i>Edit Form
        </a>
        ...
        <form action="{{ route('admin.dynamic-forms.destroy', $form->id) }}" method="POST" class="d-inline-block" onsubmit="return confirm('Are you sure you want to delete this form and all its associated fields and responses?');">
            @csrf
            @method('DELETE')
                    ...
                    <tbody>
                        @foreach ($form->responses as $response)
                            <tr>
                                ...
                                <td>
                                    @if ($response->client)
                                        <a href="{{ route('admin.clients.show', $response->client->id) }}">{{ $response->client->user->name ?? $response->client->company_name }}</a>
                                    @else
                                        Guest
                                    @endif
                                </td>
